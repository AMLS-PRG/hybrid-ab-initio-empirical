...
test -f observable.raw && split observable.raw -l $nline_per_set -d -a 3 observable.raw
...
for ii in $(seq 0 $nset_1); do
	...
	test -f observable.raw$pi && mv observable.raw$pi set.$pi/observable.raw
...
'
	python -c \
		'import numpy as np; import os.path;
if os.path.isfile("observable.raw"):
   data = np.loadtxt("observable.raw");
   data = data.astype (np.float64);
   np.save ("observable", data)
'
	rm *.raw
	cd ../
done
